...
// check phone is already exist or not
  ...
  exports.AddCustomer = (data) => {
   ...
   return new Promise((resolve, reject) => {
     ...
     sql.query(`INSERT INTO customers (first_name,last_name,phone) values ('${data.first_name}','${data.last_name}','${data.phone}')`, (err, val) => {
       if (err) reject(err);
       else resolve(val);
     });
   });
 ...
 exports.UpdateCustomer = (data) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      sql.query(`UPDATE customers SET first_name='${data.first_name}' ,  last_name='${data.last_name}' , phone='${data.phone}' WHERE id='${data.id})'`, (err, val) => {
        if (err) reject(err);
        else resolve(val);
      });
    });
